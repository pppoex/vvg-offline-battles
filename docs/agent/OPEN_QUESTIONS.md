...
> **M6 提醒**：下列「Bot AI 范围」「单人模式处理」建议在开工 M6 前确认；其余可延后。M3 已按「单人也启 sim-worker」架构落地服务器骨架，不阻塞 M4。
...
- **是否阻塞**：否（M3 服务器骨架已不含 Bot；M6/M7 前确认）
- **优先级**：中
...
- **可选方案**：
  ...
  2. **单人使用内置 sim-worker**
     - 优点：用户体验好
     - 缺点：架构复杂
- **当前假设**：方案 1（单人也启动 sim-worker）；DECISIONS 已记录；M5 启动器负责拉起
- **是否阻塞**：否
- **优先级**：中
